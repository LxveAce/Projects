...
The Anker 347 supports pass-through charging — a solar panel feeds the battery while the deck runs. The panel-mount USB-C on the case wall is the entry point. Deploy a foldable panel when stationary, plug it in, fold up and carry when moving.

### The math

| Mode | Draw | Solar needed to gain charge |
|------|------|----------------------------|
| Stealth (Pi 5 only) | ~4W | Panel > 4W |
| Wardriving | ~7.5W | Panel > 7.5W |
| Full scan (everything on) | ~15W | Panel > 15W |
| Deck off (just charging the Anker) | 0W | Any panel works |

A 20W-rated panel produces roughly 12-15W in direct sun (panel rating is lab conditions). That's enough to sustain wardriving mode in good sunlight, or charge the Anker from empty in ~8-10 hours with the deck off. The Anker 347's USB-C input accepts up to ~18W, so there's no reason to go past a 30W panel.

### Panel options

| Panel | Output | Price | Notes |
|-------|--------|-------|-------|
| BigBlue 28W | 3× USB-A | ~$55 | SunPower cells, proven outdoor. Need USB-A to USB-C cable. |
| Nekteck 28W | 2× USB-A | ~$50 | SunPower, similar to BigBlue. |
| Anker 625 24W | USB-C | ~$85 | Charges the Anker 347 natively over PD. Cleaner but pricier. |
| Generic 20-25W USB-C | USB-C | ~$30-45 | Hit or miss on quality, but cheap. |

### How it works

1. Deploy the foldable panel and angle it toward the sun
2. Run USB-C from the panel to the deck's panel-mount USB-C charging port
3. The Anker 347 charges via pass-through while powering the deck
4. Fold up and carry when done — no case mods, no permanent mounting

Set up at a park bench, campsite, rooftop, car dashboard. Deck runs in wardriving or surveillance mode while solar sustains the battery.
