# Hardware Inventory - Purchased Items

All items purchased via Amazon and specialty PCB shops. Organized by category with links to manufacturer pages, datasheets, and setup guides where available.

**Estimated Total Spend: ~$1,900 - $2,200**

---

## Microcontrollers & Development Boards

| # | Product | ASIN | Price | Manufacturer | Project(s) |
|---|---------|------|-------|-------------|------------|
| 1 | CanaKit Raspberry Pi 5 Starter Kit PRO (8GB, 128GB) | [B0CRSNCJ6Y](https://amazon.com/dp/B0CRSNCJ6Y) | ~$260 | [CanaKit](https://www.canakit.com/canakit-raspberry-pi-5-starter-kit-turbine-black.html) | Project Nomad, Kismet, general |
| 2 | CanaKit Raspberry Pi Zero 2 W Starter Kit | [B0CT1Q3PCD](https://amazon.com/dp/B0CT1Q3PCD) | ~$75 | [CanaKit](https://www.canakit.com/raspberry-pi-zero-2-w.html) / [RPi Foundation](https://www.raspberrypi.com/products/raspberry-pi-zero-2-w/) | Pwnagotchi |
| 3 | Meshnology N30 ESP32 LoRa V3 Kit (SX1262, 915MHz) | [B0F4XPYLXR](https://amazon.com/dp/B0F4XPYLXR) | ~$25-35 | [Meshnology](https://meshnology.com/) / [Heltec](https://heltec.org/project/wifi-lora-32-v3/) | Meshtastic |
| 4 | Lonely Binary ESP32 Gold Edition 3-Pack (IPEX antenna) **×2 packs (6 units)** | [B0FRXZD6VC](https://amazon.com/dp/B0FRXZD6VC) | ~$36/ea ($72 total) | [Lonely Binary](https://lonelybinary.com/en-us/products/esp32-ipex-kit) | Marauder, BLE, Flock, spares |
| 5 | ESP32 ESP-WROOM-32 Dev Board | [B07WCG1PLV](https://amazon.com/dp/B07WCG1PLV) | ~$7-10 | [Espressif](https://www.espressif.com/en/producttype/esp32-wroom-32) | Marauder, general |
| 6 | AEDIKO ESP32 GPIO Breakout Board 5-Pack | [B0D2HNT8ZR](https://amazon.com/dp/B0D2HNT8ZR) | ~$15-20 | AEDIKO | General prototyping |
| 7 | Waveshare ESP32-C5 WiFi 6 Dev Board (N16R8) | [B0G34WYL8M](https://amazon.com/dp/B0G34WYL8M) | ~$18-22 | [Waveshare](https://www.waveshare.com/esp32-c5-wifi6-kit-n16r4.htm) | **Cyberdeck: Dual-band Marauder C5 #1** (2.4+5GHz, IPEX antenna) |
| 8 | Waveshare ESP32-C5 WiFi 6 Dev Board (variant) | [B0G3Q1QS93](https://amazon.com/dp/B0G3Q1QS93) | ~$18-22 | [Waveshare](https://www.waveshare.com/product/arduino/boards-kits/esp32-c5.htm) | **Cyberdeck: Dual-band scanner C5 #2** (2.4+5GHz, IPEX antenna) |
| 9 | LILYGO T-Display-S3 ESP32-S3 1.9" TFT (ST7789, 170x320) | [B0B93NWL6G](https://amazon.com/dp/B0B93NWL6G) | ~$18-22 | [LILYGO](https://www.lilygo.cc/products/t-display-s3) | OUI-Spy, portable tools |
| 10 | DIGISHUO Raspberry Pi Zero 2 W Kit (w/ SD, cables, headers) | — | ~$35-45 | DIGISHUO / [RPi Foundation](https://www.raspberrypi.com/products/raspberry-pi-zero-2-w/) | **RaspyJack (Project 19)** |
| 11 | EC Buying BW16-Kit 3-Pack (RTL8720DN, dual-freq WiFi 2.4+5GHz, BT5.0) | — | ~$23 | EC Buying / [Realtek](https://www.realtek.com/en/products/communications-networking/item/rtl8720dn) | Dual-band WiFi/BLE projects |
| 12 | ESP32-S2U Dev Board 3-Pack (extended antennas, 4MB flash, Type-C, CP2102) | — | ~$20 | TOP DIY TECH / [Espressif](https://www.espressif.com/en/products/socs/esp32-s2) | USB-native ESP32 projects |

### Datasheets & Drivers
- **Raspberry Pi 5**: [Datasheet](https://www.raspberrypi.com/products/raspberry-pi-5/) | [Quick Start](https://www.canakit.com/quick-start/pi)
- **Raspberry Pi Zero 2 W**: [Product Brief](https://datasheets.raspberrypi.com/rpizero2/raspberry-pi-zero-2-w-product-brief.pdf) | [Documentation](https://www.raspberrypi.com/documentation/)
- **Heltec LoRa V3**: [Datasheet V3.2](https://resource.heltec.cn/download/WiFi_LoRa_32_V3/HTIT-WB32LA_V3.2.pdf) | [Wiki](https://wiki.heltec.org/docs/devices/open-source-hardware/esp32-series/lora-32/wifi-lora-32-v3/) | CP2102 driver required
- **ESP32-WROOM-32**: [Datasheet](https://www.espressif.com/sites/default/files/documentation/esp32-wroom-32_datasheet_en.pdf) | [CP2102 Driver](https://www.silabs.com/developers/usb-to-uart-bridge-vcp-drivers)
- **ESP32-C5**: [Module Datasheet](https://documentation.espressif.com/esp32-c5-wroom-1_wroom-1u_datasheet_en.pdf) | [Waveshare Wiki](https://www.waveshare.com/wiki/ESP32-C5-WIFI6-KIT-NXRX)
- **Lonely Binary ESP32**: [CH34X Driver](https://lonelybinary.com/en-us/products/esp32-ipex-kit) | [ESP32 Datasheet](https://www.espressif.com/sites/default/files/documentation/esp32_datasheet_en.pdf)
- **Arduino IDE ESP32 Setup**: [Install Guide](https://docs.espressif.com/projects/arduino-esp32/en/latest/installing.html) (Board URL: `https://dl.espressif.com/dl/package_esp32_index.json`)

---

## Displays

| # | Product | ASIN | Price | Manufacturer | Project(s) |
|---|---------|------|-------|-------------|------------|
| 1 | Waveshare 2.13" E-Ink Display HAT V4 (250x122, SPI) | [B071S8HT76](https://amazon.com/dp/B071S8HT76) | ~$13-20 | [Waveshare](https://www.waveshare.com/2.13inch-e-paper-hat.htm) | Pwnagotchi |
| 2 | ELECROW CrowPanel 5" HMI ESP32 Display (800x480, capacitive) | [B0DL5WB7RQ](https://amazon.com/dp/B0DL5WB7RQ) | ~$35 | [ELECROW](https://www.elecrow.com/esp32-display-5-inch-hmi-display-rgb-tft-lcd-touch-screen-support-lvgl.html) | IoT dashboards |
| 3 | Hosyond 7" Touchscreen IPS DSI Display for Pi x2 (800x480) | [B0D3QB7X4Z](https://amazon.com/dp/B0D3QB7X4Z) | ~$55 ea | [Hosyond](https://hosyond.com/) | Pi desktop, dashboards |
| 4 | ELECROW 5" Resistive Touch Screen TFT LCD (800x480, HDMI) | [B013JECYF2](https://amazon.com/dp/B013JECYF2) | ~$36 | [ELECROW](https://www.elecrow.com/hdmi-5-inch-800x480-tft-display-with-automatic-backlight-control.html) | Pi displays |
| 5 | ESP32 2.8" CYD Touchscreen 2-Pack (240x320, ILI9341) | [B0G4YZ7532](https://amazon.com/dp/B0G4YZ7532) | ~$15-18 | Generic (Sunton) | IoT panels, tools |
| 6 | AITRIP 4.0" ESP32 Touchscreen (320x480, ST7796) | [B0GGB5W5XK](https://amazon.com/dp/B0GGB5W5XK) | ~$15-20 | AITRIP | IoT panels, tools |
| 7 | Waveshare 1.44" LCD Display HAT (ST7735S, 128x128, SPI, joystick+3 buttons) | [B077Z7DWW1](https://amazon.com/dp/B077Z7DWW1) | ~$12-15 | [Waveshare](https://www.waveshare.com/1.44inch-lcd-hat.htm) | **RaspyJack (Project 19)** |
| 8 | Hosyond 1.3" OLED Display 5-Pack (SH1106, I2C, 128x64, blue) | — | ~$18 | Hosyond | ESP32 status displays, general |

### Datasheets & Drivers
- **Waveshare 2.13" E-Ink V4**: [Spec PDF](https://files.waveshare.com/upload/4/4e/2.13inch_e-Paper_V4_Specification.pdf) | [Wiki](https://www.waveshare.com/wiki/2.13inch_e-Paper_HAT_Manual) | [GitHub Demo Code](https://github.com/waveshareteam/e-Paper) | [Schematic](https://www.waveshare.com/wiki/File:2.13inch_e-Paper_HAT_Schematic.pdf)
- **ELECROW 5" HMI**: [User Manual](https://www.elecrow.com/download/product/ESP32_Display/ESP32_Display_HMI_User_Manual.pdf) | [Wiki](https://www.elecrow.com/wiki/ESP32_Display_5.0-inch_HMI_Arduino_Tutorial.html) | [GitHub](https://github.com/Elecrow-RD/CrowPanel-5.0-HMI-ESP32-Display-800x480)
- **ELECROW 5" HDMI**: [LCD-show driver](https://github.com/goodtft/LCD-show) | [Setup gist](https://gist.github.com/kaz-yos/992df9f479a2063cf33ab61d4660799c)
- **CYD 2.8"**: [Community GitHub](https://github.com/witnessmenow/ESP32-Cheap-Yellow-Display) | [Specs PDF](https://github.com/witnessmenow/ESP32-Cheap-Yellow-Display/blob/main/OriginalDocumentation/2-Specification/ESP32-2432S028%20Specifications-EN.pdf) | [Pinout](https://randomnerdtutorials.com/esp32-cheap-yellow-display-cyd-pinout-esp32-2432s028r/)
- **AITRIP 4.0" ST7796**: [LCD Wiki](https://www.lcdwiki.com/4.0inch_SPI_Module_ST7796) | Library: TFT_eSPI (Bodmer)

---

## Networking & Antennas

| # | Product | ASIN | Price | Manufacturer | Project(s) |
|---|---------|------|-------|-------------|------------|
| 1 | Panda Wireless PAU0F AXE3000 WiFi 6E USB 3.0 Adapter | [B0D972VY9B](https://amazon.com/dp/B0D972VY9B) | ~$60 | [Panda Wireless](https://www.pandawireless.com/pandaAXE3000dualant.html) | Kismet, pentesting |
| 2 | RT5370 WiFi USB Dongle (150Mbps, 2.4GHz) | [B0GVYVB8NR](https://amazon.com/dp/B0GVYVB8NR) | ~$8-12 | Generic (Ralink/MediaTek) | Pi WiFi, monitoring |
| 3 | 915MHz LoRa Antenna 3dBi 2-Pack + IPEX cables | [B0FRGD4MNB](https://amazon.com/dp/B0FRGD4MNB) | ~$8-12 | Generic | Meshtastic |
| 4 | Bingfu WiFi/BT Antenna 2-Pack (2.4/5.8GHz) | [B0GSTZZPWV](https://amazon.com/dp/B0GSTZZPWV) | ~$8-10 | [Bingfu](https://bingfushop.com/) | Meshtastic gateway |
| 5 | DIYmall 2.4G WiFi Antenna 2-Pack + U.FL cables | [B00ZBJNO9O](https://amazon.com/dp/B00ZBJNO9O) | ~$8 | [DIYmall](http://www.diymalls.com/) | ESP32 WiFi range |
| 6 | Boobrie RP-SMA to SMA Adapter 2-Pack | [B07D9JYMYJ](https://amazon.com/dp/B07D9JYMYJ) | ~$5-7 | [Boobrie](https://www.boobrie.net/) | Antenna adapting |
| 7 | Rydocyee IPEX U.FL to SMA Female Pigtail Cables (15cm) | — | ~$5-8 | Rydocyee | **Cyberdeck: ESP32-C5 IPEX→SMA bulkhead** |
| 8 | Cable Matters USB to Ethernet Adapter (AX88772, USB 2.0, 10/100) | — | ~$8-12 | [Cable Matters](https://www.cablematters.com/) | **RaspyJack (Project 19)** |
| 9 | Eifagur 315MHz SMA Male Antenna 2-Pack (for CC1101 / garage door openers) | — | ~$11 | Eifagur | **HaleHound (Project 18)** — CC1101 US band |

### Datasheets & Drivers
- **Panda PAU0F (MT7921A)**: [Specs](https://www.pandawireless.com/Specs%20|%20Panda%20Wireless.html) | [Windows Driver](https://www.pandawireless.com/pandaWiFi6E.htm) | [Linux in-kernel](https://github.com/morrownr/USB-WiFi)
- **RT5370**: [Drivers (Archive.org)](https://archive.org/details/ralink-rt-5370-usbwifi-drivers-manual) | [Windows Drivers](https://www.wireless-driver.com/ralink-usb-rt2870r-t2770-rt357x-rt5370-windows-drivers-ver3240/)

---

## Power & Battery

| # | Product | ASIN | Price | Manufacturer | Project(s) |
|---|---------|------|-------|-------------|------------|
| 1 | PiSugar S 1200mAh UPS Battery for Pi Zero | [B0GLYFCM2F](https://amazon.com/dp/B0GLYFCM2F) | ~$28 | [PiSugar](https://www.pisugar.com/products/pisugar-s-raspberry-pi-zero-battery) | Pwnagotchi |
| 2 | SunFounder BreadVolt Breadboard Power Supply (5V/3.3V) | [B0D31CWN3X](https://amazon.com/dp/B0D31CWN3X) | ~$10 | [SunFounder](https://www.sunfounder.com/products/sunfounder-breadvolt-breadboard-power-supply-module) | Prototyping |
| 3 | EEMB 3.7V 1000mAh LiPo Battery (JST 1.25mm, 503450) | [B08214DJLJ](https://amazon.com/dp/B08214DJLJ) | ~$8-12 | [EEMB](https://efriendsworld.com/) | **OUI-Spy** (LILYGO T-Display-S3 portable power) |

### Datasheets & Drivers
- **PiSugar S**: [Pwnagotchi Guide](https://docs.pisugar.com/docs/product-wiki/pwnagotchi/intro) | [GitHub](https://github.com/PiSugar/PiSugar) | NOTE: PiSugar S lacks I2C -- no software battery monitoring (need PiSugar 2/3 for that)
- **BreadVolt**: [Datasheet](https://mm.digikey.com/Volume0/opasdata/d220001/medias/docus/8924/CN0459D%20%20%20BreadVolt%20Breadboard%20Power%20Supply%20Module%20.pdf) | [Docs](https://docs.sunfounder.com/projects/breadvolt/en/latest/) | [GitHub](https://github.com/sunfounder/breadvolt)

---

## Storage

| # | Product | ASIN | Price | Manufacturer | Project(s) |
|---|---------|------|-------|-------------|------------|
| 1 | 128GB Micro SD Card 6-Pack (U3/A1/V30) | [B0DDGSC3PT](https://amazon.com/dp/B0DDGSC3PT) | ~$35-45 | Lerdisk/Factory Wholesale | Pi projects, Meshtastic |
| 2 | KOOTION 10-Pack 16GB Micro SD Cards (Class 10) | [B07XJVFVSJ](https://amazon.com/dp/B07XJVFVSJ) | ~$19-25 | [KOOTION](https://shop.kootion.com/) | Bulk provisioning |
| 3 | INLAND 10-Pack 32GB USB 3.0 Flash Drives | [B09LLWTMXQ](https://amazon.com/dp/B09LLWTMXQ) | ~$24-30 | INLAND (Micro Center) | Bootable OS, firmware |
| 4 | Kiorafoto 54-Slot Micro SD Card Case | [B0CQLNC22P](https://amazon.com/dp/B0CQLNC22P) | ~$15-20 | [Kiorafoto](https://www.kiorafoto.com/) | Organization |
| 5 | JJC 36-Slot SD/Micro SD Card Wallet | [B07SYPYH5M](https://amazon.com/dp/B07SYPYH5M) | ~$14 | [JJC](https://jjcfoto.com/product/jcr-sdmsd36/) | Organization |

---

## Input Devices

| # | Product | ASIN | Price | Manufacturer | Project(s) |
|---|---------|------|-------|-------------|------------|
| 1 | ProtoArc XK01 TP Foldable Bluetooth Keyboard + Touchpad | [B0FYNPT7XF](https://amazon.com/dp/B0FYNPT7XF) | ~$46 | [ProtoArc](https://www.protoarc.com/products/xk01-tp-foldable-keyboard-with-touchpad) | Field work |
| 2 | Rii K06 Mini Bluetooth Keyboard x2 (backlit, IR learning) | [B0BML42L6X](https://amazon.com/dp/B0BML42L6X) | ~$27 ea | [Rii](https://www.riimall.com/) | Headless Pi control |

### Datasheets & Drivers
- **Rii K06**: [Manual PDF](http://www.riitek.com/upload/K06BT.pdf) -- no drivers needed (Bluetooth HID)

---

## Tools & Supplies

| # | Product | ASIN | Price | Manufacturer | Project(s) |
|---|---------|------|-------|-------------|------------|
| 1 | Fluke 17B+ Digital Multimeter | [B0779621KZ](https://amazon.com/dp/B0779621KZ) | ~$155-173 | [Fluke](https://www.fluke.com/en-us/product/electrical-testing/digital-multimeters/fluke-17b-plus) | All electronics work |
| 2 | Professional Silicone Soldering Pad (17.7" x 11.8") | [B0D7MF2MPZ](https://amazon.com/dp/B0D7MF2MPZ) | ~$10-15 | Generic | Soldering workspace |
| 3 | MYJOR Kapton Tape 4-Roll Assortment (1/4" - 1") | [B07RZYY2T1](https://amazon.com/dp/B07RZYY2T1) | ~$8-12 | MYJOR | PCB protection |
| 4 | Soldering Iron + Workstation | N/A (owned) | -- | -- | All soldering work |
| 5 | Haojiaho Anti-Static ESD Cleaning Brushes 10-Pack | — | ~$5-7 | Haojiaho | PCB/component cleaning |
| 6 | Sn42/Bi58 Lead-Free Solder Paste 30g (low-temp, 138°C) | — | ~$8-10 | Generic | SMD rework, fine-pitch soldering |

### Datasheets & Drivers
- **Fluke 17B+**: [Product Page](https://www.fluke.com/en-us/product/electrical-testing/digital-multimeters/fluke-17b-plus) | [Datasheet](https://profitools.az/wp-content/uploads/2022/12/Fluke-17B-Digital-Multimeter-datasheet.pdf) | [User Manual](https://pim-resources.coleparmer.com/instruction-manual/17785-13-14-15b-17b-digital-multimeters-manual.pdf)

---

## Components & Prototyping

| # | Product | ASIN | Price | Manufacturer | Project(s) |
|---|---------|------|-------|-------------|------------|
| 1 | REXQualis Electronics Component Fun Kit (500+ pcs) | [B073ZC68QG](https://amazon.com/dp/B073ZC68QG) | ~$14 | [REXQualis](https://www.rexqualis.com/) | Learning, prototyping |
| 2 | Glarks 190Pcs 2.54mm Pin Header Connector Kit | [B09BDX69VX](https://amazon.com/dp/B09BDX69VX) | ~$12-15 | [Glarks](https://glarkshardware.shop/) | PCB connections |
| 3 | ELEGOO 6PCS 170-Point Mini Breadboard | [B09ZQPRFTB](https://amazon.com/dp/B09ZQPRFTB) | ~$7-10 | [ELEGOO](https://www.elegoo.com/) | Circuit prototyping |
| 4 | CC1101 SubGHz Wireless Transceiver 2-Pack (315/433/868/915MHz, SMA antenna) | [B01DS1WUEQ](https://amazon.com/dp/B01DS1WUEQ) | ~$22 | Generic | **HaleHound (Project 18)** — SubGHz TX/RX |
| 5 | HiLetgo NRF24L01+PA+LNA 2-Pack (2.4GHz, 1100m range, SMA antenna) | [B00WG9HO6Q](https://amazon.com/dp/B00WG9HO6Q) | ~$8-10 | [HiLetgo](https://www.hiletgo.com/) | **HaleHound (Project 18)** — 2.4GHz RF |
| 6 | HiLetgo PN532 NFC/RFID Module V3 (I2C/SPI/HSU, 13.56MHz, incl. S50 cards) | [B01I1J17LC](https://amazon.com/dp/B01I1J17LC) | ~$6-8 | [HiLetgo](https://www.hiletgo.com/) | **HaleHound (Project 18)** — NFC read/write/emulate |
| 7 | Cylewet 10Pcs 5V Active Buzzer (electromagnetic, continuous tone) | [B01N7NHSY6](https://amazon.com/dp/B01N7NHSY6) | ~$5-7 | Cylewet | **OUI-Spy** alert buzzer, general |
| 8 | HiRCTR NRF24L01+ Wireless Transceiver 4-Pack (2.4GHz) | — | ~$8 | HiRCTR / XKD store | Spares, future RF projects |
| 9 | NRF24L01 Socket Adapter Board 5-Pack (8-pin breakout) | — | ~$9 | jumu | Easy NRF24 module mounting |
| 10 | NRF24L01+ Module (bare, from PCB order) | — | ~$1.20 | WRF24010R | Spare, low-power variant |
| 11 | ALLECIN 24-Value Electrolytic Capacitor Assortment Kit (0.1uF-1000uF) | — | ~$14 | ALLECIN | General prototyping, power filtering |

---

## Custom DIY PCBs (Order #301544)

| # | Product | SKU | Price | Notes |
|---|---------|-----|-------|-------|
| 1 | ESP32-RF_Ebyte DIY-PCB | PDE25598P | $20.00 | Custom PCB for ESP32 + Ebyte RF module |
| 2 | WiFiX DIY-PCB | PDE25594P | $20.00 | Custom WiFi scanner PCB |
| 3 | ESP32-RF DIY-PCB | PIE25592P | $20.00 | Custom PCB for ESP32 + RF projects |
| 4 | C3Mini-RF | PDE25591P | $15.00 | Compact ESP32-C3 + RF PCB |

**Order Total: $157.77** (subtotal $76.20 + $46.14 shipping + $35.43 tariff)

---

## Accessories

| # | Product | ASIN | Price | Manufacturer | Project(s) |
|---|---------|------|-------|-------------|------------|
| 1 | JSAUX Micro HDMI to HDMI Adapter x3 (4K@60Hz) | [B09LYPXPH6](https://amazon.com/dp/B09LYPXPH6) | ~$9 ea | [JSAUX](https://jsaux.com/) | Pi 4/5 display |
| 2 | Meshnology Heltec V4 Case (antenna + charging cutouts) | [B0G2LQXSGY](https://amazon.com/dp/B0G2LQXSGY) | ~$8-12 | [Meshnology](https://meshnology.com/) | Meshtastic node |
| 3 | UGREEN Micro USB OTG Cable (Micro-B to USB-A female) | [B00LN3LQKQ](https://amazon.com/dp/B00LN3LQKQ) | ~$3-5 | [UGREEN](https://www.ugreen.com/) | **RaspyJack (Project 19)** — USB host mode |
| 4 | Orbic Speed RC400L (carrier unlocked, 4G LTE mobile hotspot) | [B0F45RXZCW](https://amazon.com/dp/B0F45RXZCW) | ~$75 | [Orbic](https://www.orbic.us/) | **RayHunter (Project 05)** — IMSI catcher detection |

---

## Inventory by Project

### Pwnagotchi
- [x] CanaKit Raspberry Pi Zero 2 W Starter Kit
- [x] Waveshare 2.13" E-Ink Display HAT V4
- [x] PiSugar S 1200mAh UPS Battery
- [x] Rii K06 Mini Keyboard #1 (dedicated to Pwnagotchi rig)
- [x] Fluke 17B+ Multimeter (for testing solder joints)
- [x] 16GB Micro SD Cards

### Meshtastic
- [x] Meshnology N30 ESP32 LoRa V3 Kit
- [x] 915MHz LoRa Antennas (2-pack)
- [x] Bingfu WiFi/BT Antennas (2-pack)
- [x] DIYmall WiFi Antennas (2-pack)
- [x] Meshnology Heltec V4 Case
- [x] Boobrie SMA/RP-SMA Adapters
- [x] 128GB Micro SD Cards

### ESP32 Marauder
- [x] Lonely Binary ESP32 Gold Edition (3-pack)
- [x] ESP32 ESP-WROOM-32 Dev Board
- [x] AEDIKO ESP32 GPIO Breakout Boards
- [x] ESP32 2.8" CYD Touchscreen (for GUI)
- [x] AITRIP 4.0" ESP32 Touchscreen (for GUI)
- [x] WiFi Antennas

### Project Nomad
- [x] CanaKit Raspberry Pi 5 (ARM -- incompatible, need x64)
- [ ] LattePanda or x64 SBC (not yet purchased)

### BLE Detection / Chasing Your Tail
- [x] Lonely Binary ESP32 Gold Edition
- [x] ESP32 ESP-WROOM-32

### Flock / Drone Detection
- [x] Lonely Binary ESP32 Gold Edition
- [x] ESP32 boards
- [x] WiFi antennas

### OUI-Spy
- [x] Lonely Binary ESP32 Gold #3 (WiFi-only Flock-You promiscuous mode)
- [x] CYD 2.8" Touchscreen #2 (WiFi-only with built-in display, needs custom firmware)
- [x] DIYmall 2.4G WiFi Antennas (for extended range)
- [x] Boobrie SMA/RP-SMA Adapters
- [x] LILYGO T-Display S3 (ESP32-S3, built-in 1.9" TFT, BLE+WiFi)
- [x] Cylewet 5V Active Buzzer 10-pack (note: active, not passive — user chose active)
- [x] EEMB 3.7V LiPo battery 1000mAh (JST 1.25mm)

### ESP32-C5 Dual-Band (Cyberdeck)
- [x] Waveshare ESP32-C5 WiFi 6 board #1 → **Dual-band Marauder** (2.4+5GHz, headless, controlled via Pi 5)
- [x] Waveshare ESP32-C5 WiFi 6 board #2 → **Dual-band scanner/wardriving** (2.4+5GHz, headless)
- [x] Bingfu dual-band antennas (2.4/5.8GHz) → C5 boards via IPEX→SMA
- [x] Rydocyee U.FL-to-SMA pigtail cables (for C5 IPEX → SMA bulkhead)

### HaleHound (Project 18)
- [x] ESP32 2.8" CYD Touchscreen #2 → **HaleHound multi-protocol toolkit**
- [x] Micro SD card (loot + custom creds)
- [x] CC1101 SubGHz module 2-pack (315/433/868/915MHz + included 433MHz SMA whip)
- [x] HiLetgo NRF24L01+PA+LNA 2-pack (2.4GHz, 1100m range, included RP-SMA duck antenna)
- [x] HiLetgo PN532 V3 NFC/RFID module (I2C/SPI/HSU, built-in PCB coil antenna, includes S50 cards)
- [x] 915MHz SMA whip antenna (for CC1101 — ISM band) — reuse spare from LoRa 2-pack (Networking #3)
- [x] 315MHz SMA whip antenna 2-pack (Eifagur, for CC1101 — US Tesla/garage frequency)

### RaspyJack (Project 19)
- [x] DIGISHUO Raspberry Pi Zero 2 W Kit (w/ SD, cables, headers)
- [x] Waveshare 1.44" LCD HAT (ST7735S, 128x128, joystick + buttons)
- [x] UGREEN Micro USB OTG Cable
- [x] Cable Matters USB-to-Ethernet Adapter (AX88772, Linux compatible)

### RayHunter (Project 05 — Cyberdeck Integrated)
- [x] Orbic Speed RC400L (carrier unlocked, 4G LTE hotspot) — $74.89
- [ ] Deactivated SIM card (any carrier, no active plan needed) — free/~$1

### Kismet Wardriving
- [x] CanaKit Raspberry Pi 5 Starter Kit PRO
- [x] Panda Wireless PAU0F WiFi 6E Adapter
- [x] RT5370 WiFi USB Dongle (secondary)
- [x] Hosyond 7" Touchscreen DSI Display #1 (dedicated dashboard)
- [x] Rii K06 Mini Keyboard #2 (field kit)
- [x] JSAUX HDMI Adapter #2 (Pi 5)
- [x] 128GB Micro SD Cards
- [ ] GPS Module (not yet purchased)

### General / Shared
- [x] Fluke 17B+ Multimeter
- [x] Soldering Iron + Workstation (owned, not from Amazon)
- [x] Silicone Soldering Pad
- [x] Kapton Tape
- [x] ESD Brushes (10-pack)
- [x] Solder Paste (Sn42/Bi58 low-temp)
- [x] REXQualis Component Kit
- [x] Pin Headers, Breadboards
- [x] ProtoArc Foldable Keyboard
- [x] JSAUX HDMI Adapters (3x — Pi 5, Pi Zero, spare)
- [x] USB Flash Drives (10-pack)
- [x] SD Card Cases

---

## Quantity Impact Analysis

Some items were purchased in multiples, allowing dedicated assignment to separate rigs:

| Item | Qty | Assignment |
|------|-----|-----------|
| Rii K06 Mini Keyboard | 2 | #1 → Pwnagotchi rig (headless Pi Zero setup/troubleshooting) · #2 → Field kit (Kismet wardriving / general) |
| Hosyond 7" DSI Display | 2 | #1 → Kismet wardriving dashboard (Pi 5) · #2 → General Pi desktop / Project Nomad (if LattePanda not used) |
| JSAUX Micro HDMI Adapter | 3 | #1 → Pi 5 · #2 → Pi Zero 2 W · #3 → Spare |
| Lonely Binary ESP32 Gold | 6 (2× 3-packs) | #1 → Marauder 2.4GHz + CYD #1 (cyberdeck SW1) · #2 → Flock detection (cyberdeck SW2) · #3 → BLE/CYT scanner (cyberdeck SW3) · #4-6 → Spares for future projects |
| ESP32 2.8" CYD Touchscreen | 2 | #1 → Marauder touchscreen (built) · #2 → HaleHound multi-protocol toolkit (cyberdeck SW8) |
| CC1101 SubGHz Transceiver | 2 | #1 → HaleHound (cyberdeck SW8) · #2 → Spare/future project |
| NRF24L01+PA+LNA | 2 | #1 → HaleHound (cyberdeck SW8) · #2 → Spare/future project |
| Cylewet Active Buzzer | 10 | #1 → OUI-Spy alert · #2-10 → Spares for future ESP32 projects |
| ESD Brushes | 10 | General PCB/component cleaning across all projects |
| Eifagur 315MHz SMA Antenna | 2 | #1 → HaleHound CC1101 (US Tesla/garage) · #2 → Spare |
| NRF24L01+ (HiRCTR basic) | 4 | Spares for future 2.4GHz RF projects |
| NRF24L01 Socket Adapter | 5 | Easy mounting for NRF24 modules across projects |
| Hosyond 1.3" OLED Display | 5 | Status displays for ESP32/Pi projects |
| EC Buying BW16-Kit (RTL8720DN) | 3 | Dual-band WiFi/BT5.0 dev boards |
| ESP32-S2U Dev Board | 3 | USB-native ESP32 projects |
